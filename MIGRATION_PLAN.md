...
The goal of this project is to migrate all relevant academic materials (past exams, summaries, exercises, and TTTs) from the legacy **Seafile 13** repository (`<seafile-host>`) into the new **Burgieclan** production platform (`liv`), backed by **Hetzner Object Storage (S3)** and **PostgreSQL 18**.
...
```mermaid
flowchart TD
    subgraph Source ["Source Server: <seafile-host> (Seafile 13)"]
        S_DB["MariaDB 10.11\n(seafile-db)"]
...
Run a Python crawler script inside the `seafile` container on `<seafile-host>`:
...
| **1** | `01_crawl_seafile.py` | `<seafile-host>` (Seafile) | `manifest_raw.jsonl` |
