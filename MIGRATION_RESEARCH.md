...
This document compiles the architectural analysis, data schemas, API interfaces, and structural findings gathered from the live systems (**Seafile 13** on `10.10.10.27` and **Burgieclan Production** on `liv`).
...
### 2.1 Seafile Architecture on `10.10.10.27`
...
Analysis of `seafile-db` on `10.10.10.27` reveals:
...
```mermaid
flowchart TD
    A["Seafile 13 (10.10.10.27)"] -->|"seafile_api (Python RPC)"| B["Crawler & Manifest Generator"]
