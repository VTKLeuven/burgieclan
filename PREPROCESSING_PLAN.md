...
Using specialized subagents allows us to parallelize work across both servers (`<seafile-host>` and `liv`) and keep massive file lists isolated from the main conversation context:
...
| :--- | :--- | :--- | :--- |
| **1. Seafile Crawler** | Seafile Data Extractor | `<seafile-host>` | Runs Python script via `seafile_api` inside container; crawls all 30 parent libraries recursively; generates `manifest_raw.jsonl` with file sizes, mtimes, object IDs, and SHA-256 hashes. |
...
| **4. Staging Coordinator** | File Transfer & Integrity | `<seafile-host>` $\rightarrow$ `liv` | Sets up `/mnt/immich/burgieclan-staging/`; streams cleaned unique files from Seafile to `liv`; verifies SHA-256 checksums on completion. |
...
- **Step 1.1**: Agent 1 runs `scripts/migration/01_crawl_seafile.py` on `<seafile-host>` across all 30 root libraries.
...
- **Step 4.2**: Streams the cleaned, deduplicated files from `<seafile-host>` to `/mnt/immich/burgieclan-staging/`.
