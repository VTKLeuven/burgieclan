...
Two different machines both report the hostname `burgieclan`. They are not the
same box and mixing them up is easy.

| SSH target | What it is |
| --- | --- |
| `liv` | **Burgieclan production.** `/opt/burgieclan` (compose, `data/`, `jwt/`, `postgres_data/`). Root fs is ~87% full; `/mnt/immich` (NFS, ~9 TB free) is the only place with room for backups. No passwordless sudo; `it` is in the `docker` group. |
| `burgieclan` (`burgieclan.leu1.vtk.be`) | **Old Seafile 6.1.1**, Ubuntu 18.04, Python 2.7, native install at `/vtk/burgieclan`. `/vtk` has 1.6 TB free. `seafile-data` is mode 0700 and `it` has no sudo, so nothing there is readable without root. |
| `10.10.10.27` (no DNS; reached via `yana`) | **Seafile 13.0.25**, Dockerised, MariaDB, serving `burgieclan-backup.vtk.be`. Holds the 56 shared libraries. Passwordless sudo, `it` in `docker` group. |
| `yana` | Not a server — an **sshpiper** username-routing SSH gateway. Hosts nothing. |
...
without root, and the live copy is a Dockerised Seafile 13 on `10.10.10.27`, so
