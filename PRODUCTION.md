...
> **Note**: The `S3_*` variables are only consumed in production (`when@prod` in `flysystem.yaml`). They are read from the server's `.env` by `docker-compose.prod.yml`, **not** from GitHub Actions secrets. Because the compose file uses the fail-fast form (`${S3_ACCESS_KEY:?...}`), a deploy will abort at `docker compose up` if any of them are missing.
...
> The upload size limit and allowed MIME types used to be the `NEXT_PUBLIC_MAX_FILE_SIZE_MB` and
> `NEXT_PUBLIC_ALLOWED_MIME_TYPES` env vars. They are no longer read from the environment — see
> [File Uploads](#file-uploads) for where they live now.
...
- **Size**: 200MB. This is not configurable at runtime; it is set in four places that must be changed together:
  ...
  `docker compose exec backend php -i | grep upload_max_filesize` when debugging upload failures.
- **MIME types**: `ALLOWED_MIME_TYPES` in `frontend/src/utils/constants/upload.ts`, mirrored by the `mimeTypes` constraint in `src/Controller/Admin/DocumentBulkUploadController.php`
- **Storage**: In **production**, uploads are stored in Hetzner Object Storage (S3-compatible) under the `documents/` prefix, via Flysystem + VichUploader (`flysystem.yaml`, `vich_uploader.yaml`, `when@prod`). In **dev/test**, uploads go to the local `data/documents` directory. Filenames are generated by `SmartUniqueNamer`, so the object key in the bucket is not the original upload name — the real name is stored in the `document.file_name` column.
